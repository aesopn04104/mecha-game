...
function repairAlliesFromPlayerRepair(playerRepairAmount) {
    let allyRepairAmount = Math.floor(playerRepairAmount / 2);
    ...
    allies.forEach(ally => {
        ...
        let beforeHp = ally.hp;
        ally.hp = Math.min(ally.maxHp, ally.hp + allyRepairAmount);

        if (ally.hp > beforeHp) {
            write(`${ally.name} 同步維修，HP 從 ${beforeHp} 回復到 ${ally.hp}。`);
        }
    });
}
...
function repairMech() {
    ...
    write(`維修完成：你的機體回復 ${repairAmount} HP。`);
    repairAlliesFromPlayerRepair(repairAmount);
    updateUI();
}
